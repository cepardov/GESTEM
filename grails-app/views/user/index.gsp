        ...
        <table class="responsive-table bordered highlight-2 centered">
            ...
            <g:each var="v" in="${userList}">
                <tr onclick="location = '<g:createLink controller="user" action="show" id="${v.id}" params="[name : v.nombre, lastName : v.paterno]"/>'">
                    ...
                    <td>${v.materno}</td>
                    <td>
                        <g:link class="btn-floating waves-effect waves-light blue tooltipped" action="show"
                        ...
                        <g:link class="btn-floating waves-effect waves-light red tooltipped" action="eliminar"
                                id="${v.id}" data-position="left" data-delay="50" data-tooltip="Eliminar Usuario"><i
                        ...
                        <div class="input-field col s12 m2">
                            <select name="institucion" required="" id="institucion">
                                <option value="" disabled
                                        <g:if test="${!sec.loggedInUserInfo(field: 'institucion')}">selected</g:if>>Seleccione Institucion</option>
                                <g:if test="${!sec.loggedInUserInfo(field: 'institucion')}">
                                    <option value="">Aplicación</option>
                                </g:if>
                                <g:each var="v" in="${institucionList}">
                                    <option value="${v.id}"
                                            <g:if test="${v.name == sec.loggedInUserInfo(field: 'institucion')}">selected</g:if>>${v.name}</option>
                                </g:each>
                            </select>
