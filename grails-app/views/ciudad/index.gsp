...
<div class="slider col s12 m12">
    <div id="list-ciudad" class="content scaffold-list" role="main">
            ...
            <table class="responsive-table bordered highlight-2 centered">
                ...
                <g:each var="v" in="${ciudadList}">
                    <tr onclick="location='<g:createLink controller="comuna" action="index" params="[paisId: params.paisId, regionId: params.regionId, ciudadId : v.id, ciudadName : v.name]"/>'">
                        ...
                        <td>${v.name}</td>
                        <td>
                            <g:link class="btn-floating waves-effect waves-light yellow darken-2 tooltipped" id="${v.id}" params="[regionId : params.regionId, regionName : params.regionName]" data-position="left" data-delay="50" data-tooltip="Editar ${controllerName}"><i class="material-icons">edit</i></g:link>
        ...
        <div class="pagination">
            <g:paginate total="${ciudadCount ?: 0}" />
        </div>
    </div>
</div>
<!-- Menu flotante Crear -->
...
<div id="modalCreate" class="modal bottom-sheet">
    <div class="modal-content">
        ...
        <div class="row">
        <!---rut, nombre, paterno, clave, materno, valor, estado, ciudad, valorHoraExtra--->
            <g:form action="save">
                <div class="row">
                    <div class="input-field col s12 m2">
                        <f:input property="code" id="code" bean="ciudad"/>
                        <label for="code">Codigo</label>
                    </div>
                    <div class="input-field col s12 m2">
                        <f:input property="name" id="name" bean="ciudad"/>
                        <label for="name">Nombre</label>
                    </div>
                    <div class="input-field col s12 m2">
                        <select name="region.id" required="" id="region">
                            <option value="" disabled <g:if test="${!params.regionName}">selected</g:if>>Seleccione Pais</option>
                            <g:each var="v" in="${regionList}">
                                <option value="${v.id}" <g:if test="${v.name == params.regionName}">selected</g:if>>${v.name}</option>
                            </g:each>
                        </select>
...
<div id="modalEdicion" class="modal bottom-sheet">
    <div class="modal-content">
        ...
        <div class="row">
        <!---rut, nombre, paterno, clave, materno, valor, estado, ciudad, valorHoraExtra--->
            <g:form class="col s12" resource="${this.ciudad}" method="PUT">
                <div class="row">
                    <div class="input-field col s12 m2">
                        <f:input property="code" id="code" bean="ciudad"/>
                        <label for="code">Codigo</label>
                ...
                <div class="fixed-action-btn">
                    ...
                    <g:link action="index" params="[regionId : params.regionId, regionName : params.regionName]" class="modal-action modal-close waves-effect waves-light btn-floating btn-large red tooltipped" href="#!" data-position="left" data-delay="50" data-tooltip="Cancelar"><i class="material-icons">cancel</i></g:link>
                </div>
            </g:form>
        </div>
    </div>
